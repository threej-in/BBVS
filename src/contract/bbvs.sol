...
//geth --http --http.corsdomain="package://6fd22d6fe5549ad4c4d8fd3ca0b7816b.mod",* --http.api web3,eth,debug,personal,net --vmdebug --datadir /tmp/eth/12 --allow-insecure-unlock --dev console
//["BJP","AAP","NOTA"]
...
contract BBVS is inheritingBase{
    ...
    */
    ...
    modifier isPollActive(uint _pid) {
        ...
        uint expdate = (polls[_pid].periodInDays * 86400) + polls[_pid].time;
        if(block.timestamp > expdate){
            polls[_pid].status = Status.Inactive;
            revert("Poll has been expired!");
        }
        _;
    }
